...
export const runSelection = (
  ...
  const harness = `
set -u
test_root=$(mktemp -d /tmp/postlude-preview-test.XXXXXX)
...
case "$endpoint" in
  ...
  */actions/workflows/343866633/runs*) printf '%s' "$SELECT_CANDIDATES" ;;
...
status=$?
set -e
printf '\n__POSTLUDE_OUTPUTS__\n'
cat "$GITHUB_OUTPUT"
printf '__POSTLUDE_LOG__\n'
cat "$SELECT_GH_LOG"
printf '__POSTLUDE_END__\n'
exit "$status"
`;
  ...
  const result = globalThis.Bun.spawnSync(['bash', '-c', harness], {
    ...
    env: selectionEnvironment([
      ...
      ['EVENT_WORKFLOW_ID', '343866633'],
      ['GH_TOKEN', 'test-token'],
      ['REPOSITORY', 'davidvornholt/postlude'],
  ...
  const outputText = stdout
    .split('__POSTLUDE_OUTPUTS__\n', 2)[1]
    ?.split('__POSTLUDE_LOG__\n', 1)[0];
  const log = stdout
    .split('__POSTLUDE_LOG__\n', 2)[1]
    ?.split('__POSTLUDE_END__\n', 1)[0];
