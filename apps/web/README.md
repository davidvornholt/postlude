...
GitHub OAuth keeps signed-in production routes outside that scan. Two isolated browser fixtures cover the signed-in writing UI without pretending to test authentication or database loaders. The writing-page fixture server-renders and hydrates the real `DayPage` with production CSS and viewport metadata. It exercises keyboard order, target size, computed Markdown heading styles, and saved, saving, failed, invalid, expired-session, and retry states in desktop Chromium and Pixel 7. The navigation fixture mounts the real `AppShell` and `DayPage` beneath multiple browser-history routes, then proves Previous and Next update the URL, title, heading, and focus without moving focus on initial load.
