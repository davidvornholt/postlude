...
- `autosave-registry.ts` owns the set of coordinators that still have a subscriber or browser work. It reuses one coordinator per date and evicts it once the day is clean and unobserved.
...
- `ui/` owns the writing page. `use-autosave.ts` adapts the coordinator to React subscriptions and browser lifecycle signals. It does not own the save queue, recovery copy, or confirmed revisions. `markdown-editor.tsx` is the Tiptap surface, where Markdown is typed and set in place rather than previewed.
