...
- `ui/` owns the writing page and archive. `use-autosave.ts` adapts the coordinator to React subscriptions and browser lifecycle signals. It does not own the save queue, recovery copy, or confirmed revisions. `markdown-editor.tsx` is the Tiptap surface, where Markdown is typed and set in place rather than previewed.
