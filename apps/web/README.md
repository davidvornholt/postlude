...
GitHub OAuth keeps signed-in production routes outside that scan. Four isolated browser fixtures cover the signed-in UI without pretending to test authentication or database loaders. The writing-page fixture server-renders and hydrates the real `DayPage` with production CSS and viewport metadata. It exercises keyboard order, target size, computed Markdown heading styles, and saved, saving, failed, invalid, expired-session, and retry states in desktop Chromium and Pixel 7. The browser-history fixture mounts the real `AppShell` and `DayPage` beneath multiple client routes, then proves Previous and Next update the URL, title, heading, and focus without moving focus on initial load. The archive-page fixture server-renders and hydrates the real `ArchivePage`; it scans empty, filled, expanded-table, and named-year states in both colour schemes on desktop and mobile. The archive-navigation fixture mounts the real `AppShell`, `DayPage`, and `ArchivePage`. It proves the first archive read waits for a pending save, then checks edits that arrive while the read is pending. A stored edit triggers a fresh read, while a failed save leaves the writing page open. It also proves a failed pre-navigation read falls through to the route's safe error page.

Server-rendered tests keep the boundaries a browser fixture does not own. `src/routes/_app.test.tsx` asserts the shell's current-page marker, landmark, skip link, and sign-out states. `src/features/journal/ui/day-page.test.tsx` owns the pre-hydration day markup and content rules. `src/routes/_app/page-measures.test.tsx` owns the exact column wrappers for writing and archive pages. The two tests under `src/routes` are colocated beside the routes they cover, which the route generator would otherwise read as route files, so `tsr.config.json` sets `routeFileIgnorePattern` to skip `*.test.ts` and `*.test.tsx` there. That file is read by both the Vite plugin and `bun run generate-routes`, so the setting is stated once.
