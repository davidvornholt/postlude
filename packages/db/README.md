...
- The search columns on `entry` keep one app-owned contract together. The web app derives `journal_search_text`, `scripture_search_text`, and `scripture_reference_search_text` from the same shared Tiptap parse model it renders, then derives canonical `search_token_text` from those raw projections. It writes all four with the entry and sets `search_projection_revision` to the row's new `revision`; the `entry_search_projection_current` check rejects a write that leaves the projections behind. PostgreSQL generates `search_vector` from the already canonical tokens with `array_to_tsvector(string_to_array(search_token_text, ' '))`, so it neither reparses the raw text nor applies a second normalization policy. The query matches every canonical token as a prefix, and the GIN index makes that match a lookup rather than a table scan. The raw projections remain separate because result excerpts must show and attribute the text behind every indexed token.
