...
- **An eyebrow announces a section.** Small letterspaced capitals are how a section opens and how a control names itself, with a hairline rule where one part of the page has to be separated from the next. The navigation links, the sign-out control, and the sign-in page's opening line are set that way today; the navigation and the sign-out control draw their own rule as part of their hover and current-page behaviour.
