...
Nothing renders the ramp today. The heatmap that did — a year grid with a described summary and a collapsed daily table as its accessible alternative — was reviewed and then removed with the rest of the design-comparison pages; it lives in commit `232938c`, at `apps/web/src/features/design-comparison/ui/heatmap.tsx`. The archive work should start from that pattern rather than from scratch.
...
WCAG 2.2 AA is a floor the design is built to, not a pass applied afterwards. Every token pair that can carry normal-size text clears 4.5:1, and every activity mark clears 3:1 against its ground. `packages/ui/src/theme-contract.test.ts` recomputes both from the token values, so it also holds pairs no page has rendered yet. `apps/web/a11y/routes.a11y.ts` scans every production route reachable without signing in under both colour schemes on desktop and mobile. GitHub OAuth keeps the signed-in production routes outside that scan. The isolated writing-page fixture server-renders and hydrates the real day component with production styles and viewport metadata, then covers keyboard order, target size, computed Markdown typography, and save states in desktop Chromium and Pixel 7. A separate browser-history fixture mounts the real signed-in shell and day page under multiple client routes to cover focus after Previous and Next navigation. Server-rendered tests still own the markup before hydration. Nothing is communicated by colour alone.
